...
class MarkdownParser:
    ...
    def __init__(self, parser_config: Optional[Dict] = {'remove_hyperlinks': True, 'remove_images': True}):
        """Init params."""
        self._parser_config = parser_config

    def init_parser(self) -> Dict:
        """Init parser and store it."""
        parser_config = self._init_parser()
        self._parser_config = parser_config

    def markdown_to_dict(self, markdown_text):
      ...
      for line in lines:
        ...
        if header_match:
          if current_header is not None:
            if current_text == '' or None: continue
    ...
    def remove_hyperlinks(self, content):
      """Get a dictionary of a markdown file from its path"""
      pattern = r"\[(.*?)\]\((.*?)\)"
    ...
    def _init_parser(self) -> Dict:
        ...
        parser_config = {'remove_hyperlinks': True, 'remove_images': True}
        if self._parser_config:
            parser_config.update(self._parser_config)
