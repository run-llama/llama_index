...
from .entities import Article, ArticleSlim, Category, ProjectVersion
...
class Document360Reader(BaseReader):
    ...
    def create_category_entity_from_document360_response(self, category_response):
        return Category(
            id=category_response["id"],
            name=category_response["name"],
            child_categories=category_response["child_categories"],
            articles=category_response["articles"],
            description=category_response["description"],
            project_version_id=category_response["project_version_id"],
            order=category_response["order"],
            parent_category_id=category_response["parent_category_id"],
            hidden=category_response["hidden"],
            icon=category_response["icon"],
            slug=category_response["slug"],
            language_code=category_response["language_code"],
            category_type=category_response["category_type"],
            created_at=category_response["created_at"],
            modified_at=category_response["modified_at"],
            status=category_response["status"],
            content_type=category_response["content_type"],
        )
    ...
    def _process_category_recursively(self, category: Category, parent_categories=[]):
        if self.should_process_category and not self.should_process_category(
            ...
            for _child_category in category.get_child_categories():
                child_category = self.create_category_entity_from_document360_response(
                    _child_category
                )
        ...
        for _article_slim in articles:
            article_slim = None
            article = None

            try:
                article_slim = ArticleSlim(
                    id=_article_slim["id"],
                    title=_article_slim["title"],
                    modified_at=_article_slim["modified_at"],
                    public_version=_article_slim["public_version"],
                    latest_version=_article_slim["latest_version"],
                    language_code=_article_slim["language_code"],
                    hidden=_article_slim["hidden"],
                    status=_article_slim["status"],
                    order=_article_slim["order"],
                    slug=_article_slim["slug"],
                    content_type=_article_slim["content_type"],
                    translation_option=_article_slim["translation_option"],
                    is_shared_article=_article_slim["is_shared_article"],
                )
                ...
                article_response = self._fetch_article(article_slim.get_id())
    ...
    def _fetch_articles(self):
        ...
        for _project_version in project_versions:
            ...
            categories_response = self._get_categories(project_version.get_id())
            categories = self._get_document360_response_data(categories_response)

            for _category in categories:
                category = self.create_category_entity_from_document360_response(
                    _category
                )
    ...
    def article_to_document(self, article: Article):
        return Document(
            ...
            text=article.get_html_content(),
            extra_info={
                "title": article.get_title(),
                "content": article.get_content(),
                "category_id": article.get_category_id(),
                "project_version_id": article.get_project_version_id(),
                "version_number": article.get_version_number(),
                "public_version": article.get_public_version(),
                "latest_version": article.get_latest_version(),
                "enable_rtl": article.get_enable_rtl(),
                "hidden": article.get_hidden(),
                "status": article.get_status(),
                "order": article.get_order(),
                ...
                "is_fall_back_content": article.get_is_fall_back_content(),
                "description": article.get_description(),
                "category_type": article.get_category_type(),
                "content_type": article.get_content_type(),
                "is_shared_article": article.get_is_shared_article(),
                "translation_option": article.get_translation_option(),
                "url": article.get_url(),
            },
        )
