...
class CrwWebReader(BasePydanticReader):
    ...
    fastCRW is an open-source web scraper that ships as a single binary. It
    can run against the managed cloud (default `https://api.fastcrw.com`), a
    self-hosted server via `api_url`, or a local zero-config engine by setting
    `CRW_LOCAL=1` in the environment (no server and no key required).

    Args:
        api_key (Optional[str]):
            The fastCRW API key. Optional: the underlying client reads
            `CRW_API_KEY` from the environment, a self-hosted server may
            require no auth at all, and `CRW_LOCAL=1` needs no key.
        api_url (Optional[str]):
            Optional base URL for a self-hosted fastCRW deployment. Also read
            from `CRW_API_URL`. Defaults to the managed cloud when omitted.
        mode (Optional[str]):
        ...
        params (Optional[dict]): The parameters to pass to the fastCRW API,
            e.g. `formats`, `onlyMainContent`, `maxDepth`, `maxPages`.

    For more details, visit: https://docs.fastcrw.com/api-reference
    ...
    def _import_crw(self) -> Any:
        try:
            ...
            raise ImportError("crw not found, please run `pip install crw`") from exc
        return CrwClient
    ...
    def _scrape_from_dict(self, crw_docs: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
    ...
    def _scrape_from_obj(self, crw_docs: Any) -> Tuple[str, Dict[str, Any]]:
        text_value = (
            self._safe_get_attr(
                crw_docs,
                "markdown",
    ...
    def _handle_scrape_response(self, crw_docs: Any) -> Tuple[str, Dict[str, Any]]:
    ...
    def __init__(
        ...
        # cloud (https://api.fastcrw.com) and reads CRW_API_KEY from env.
        client_kwargs: Dict[str, Any] = {}
        if api_key is not None:
            client_kwargs["api_key"] = api_key
        if api_url is not None:
    ...
    def load_data(
        ...
        if self.mode == "scrape":
            # [SCRAPE] https://docs.fastcrw.com/api-reference (POST /v1/scrape)
            if url is None:
            ...
            # [CRAWL] https://docs.fastcrw.com/api-reference (POST /v1/crawl)
            if url is None:
            ...
            # [MAP] https://docs.fastcrw.com/api-reference (POST /v1/map)
            ...
            # [SEARCH] https://docs.fastcrw.com/api-reference (POST /v1/search)
            if query is None:
