...
from llama_index.core.schema import BaseNode, TextNode, NodeRelationship, TransformComponent
    ...
    def _split_text_into_messages(self, text: str) -> List[Dict[str, str]]:
        ...
        for line in lines:
            if line.startswith('user:') or line.startswith('assistant:'):
                ...
                messages.append({'author_role': author_role.strip(), 'text': message_text.strip()})
        return messages

    def _process_messages(self, messages: List[Dict[str, str]], messages_metadata: List[Dict[str, Any]], doc: BaseNode) -> List[TextNode]:
        ...
        for idx, message in enumerate(messages):
            ...
            message_metadata = next((m for m in messages_metadata if m['author_role'] == author_role and m['text'] == message_text), {})
            metadata = {
                **doc.metadata,
                **message_metadata,
            }
            ...
            if author_role == 'assistant' and idx > 0:
                ...
                if context_message['author_role'] == 'user':
                    ...
                    context_metadata = {
                        **doc.metadata,
                        **next((m for m in messages_metadata if m['author_role'] == 'user' and m['text'] == context_text), {}),
                    }
                    ...
                    if context_nodes and message_nodes:
                        for i in range(len(context_nodes) - 1):
                            context_nodes[i].relationships[NodeRelationship.NEXT] = context_nodes[i + 1].as_related_node_info()
                            context_nodes[i + 1].relationships[NodeRelationship.PREVIOUS] = context_nodes[i].as_related_node_info()
                        context_nodes[-1].relationships[NodeRelationship.NEXT] = message_nodes[0].as_related_node_info()
                        message_nodes[0].relationships[NodeRelationship.PREVIOUS] = context_nodes[-1].as_related_node_info()
            ...
            for i in range(len(message_nodes) - 1):
                message_nodes[i].relationships[NodeRelationship.NEXT] = message_nodes[i + 1].as_related_node_info()
                message_nodes[i + 1].relationships[NodeRelationship.PREVIOUS] = message_nodes[i].as_related_node_info()

            # Set up relationships with previous node
            if prev_node and message_nodes:
                message_nodes[0].relationships[NodeRelationship.PREVIOUS] = prev_node.as_related_node_info()
                prev_node.relationships[NodeRelationship.NEXT] = message_nodes[0].as_related_node_info()
        ...
        for idx, token in enumerate(tokens):
            ...
            if token.type == "heading_open":
                if current_text.strip():
                    nodes.append(self._build_text_node(
                        current_text, metadata, "/".join(header_path), ref_doc_id, source_node
                    ))
                    current_text = ""
