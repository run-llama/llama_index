...
def test_real_cloudflare_ai_gateway_with_openai_and_claude():
    ...
    print("Note: Cloudflare AI Gateway automatically tried OpenAI first, then Claude if needed") 
...
def test_cloudflare_ai_gateway_fallback_when_openai_fails():
    ...
    print("Note: OpenAI failed with invalid key, but Claude handled the request successfully")
    ...
    try:
        with httpx.Client(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                print("[FAIL] Authentication failed - check your API key and permissions")
                print(f"Response: {response.text}")
            elif response.status_code == 404:
                print("[FAIL] Gateway not found - check your account ID and gateway name")
                print(f"Response: {response.text}")
            else:
...
def test_cloudflare_ai_gateway_comprehensive_methods():
    ...
    test_results.append(("Basic Chat", "PASS", chat_response.message.content[:50] + "..."))
    ...
    assert completion_response.text is not None
    assert len(completion_response.text) > 0
    test_results.append(("Basic Completion", "PASS", completion_response.text[:50] + "..."))
    ...
    stream_completion_response = llm.stream_complete("Write a short sentence about technology.")
    ...
    test_results.append(("Stream Completion", "PASS", stream_completion_content[:50] + "..."))
    ...
    system_messages = [
        ChatMessage(role="system", content="You are a helpful assistant that always responds with 'Hello from AI Gateway!'"),
    ...
    test_results.append(("System Chat", "PASS", system_chat_response.message.content[:50] + "..."))
    ...
    test_results.append(("Temperature Completion", "PASS", temp_completion_response.text[:50] + "..."))
    
    # Test 9: Chat with max_tokens parameter
    print("Testing chat with max_tokens parameter...")
    max_tokens_messages = [ChatMessage(role="user", content="Explain quantum computing in detail.")]
    ...
    test_results.append(("Max Tokens Chat", "PASS", max_tokens_chat_response.message.content[:50] + "..."))
