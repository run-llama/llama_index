...
def _duplicate_tool_call_chunks():
    ...
    Mirrors Ollama's real streaming shape: each tool call arrives whole,
    in its own chunk, with no repeats.
    """
...
def test_stream_chat_keeps_distinct_tool_calls_with_identical_arguments() -> None:
    ...
    Regression test for #22464: stream_chat() used to dedupe accumulated tool calls by
