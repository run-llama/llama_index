...
if __name__ == "__main__":
    llm = Gemini(
        model="models/gemini-2.0-flash-001", api_key=os.environ["GOOGLE_API_KEY"]
    )
    # # complete + acomplete
    # print(llm.complete("Write a poem about a magic backpack"))
    # print(asyncio.run(llm.acomplete("Write a poem about a magic backpack")))

    # # chat + achat
    # print(
    #     llm.chat(messages=[ChatMessage(content="Write a poem about a magic backpack")])
    # )
    # print(
    #     asyncio.run(
    #         llm.achat(
    #             messages=[ChatMessage(content="Write a poem about a magic backpack")]
    #         )
    #     )
    # )

    # # stream chat + stream achat
    ...
    # stream complete + stream acomplete
    response = llm.stream_complete("Write a poem about a magic backpack")
    for chunk in response:
        print(chunk)

    async def gen():
        response = await llm.astream_complete("Write a poem about a magic backpack")
    ...
    # structured generation (structured_predict)
    # simple
    ...
    schema = llm.structured_predict(output_cls=Schema, prompt=prompt)

    print(schema.schema_name)

    # with as_structured_llm
    print(
        llm.as_structured_llm(output_cls=Poem, prompt=prompt).complete(
            "Write a poem about a magic backpack"
        )
    )
    print(
        llm.as_structured_llm(output_cls=Schema, prompt=prompt).complete(
