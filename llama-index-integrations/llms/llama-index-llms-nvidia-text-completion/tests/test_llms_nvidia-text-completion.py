...
def test_validates_api_key_is_present() -> None:
    with CachedNVIDIApiKeys(set_fake_key=True):
        assert NVIDIATextCompletion()

        os.environ["NVIDIA_API_KEY"] = ""

        assert NVIDIATextCompletion(api_key="nvai-" + "x" * 9 + "-" + "x" * 54)


def test_metadata() -> None:
    assert isinstance(NVIDIATextCompletion().metadata, LLMMetadata)
