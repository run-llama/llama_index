...
def upstash_chat_store() -> UpstashChatStore:
    return UpstashChatStore(redis_url=os.environ.get("UPSTASH_REDIS_REST_URL") or "", redis_token=os.environ.get("UPSTASH_REDIS_REST_TOKEN") or "")
...
def test_delete_messages(upstash_chat_store: UpstashChatStore):
    messages = [
        ChatMessage(content="Message to delete", role="user")
    ]
    key = "test_delete_key"
    upstash_chat_store.set_messages(key, messages)
