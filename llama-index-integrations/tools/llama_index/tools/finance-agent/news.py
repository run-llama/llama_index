...
def get_current_undervalued_growth_stocks() -> pd.DataFrame:
    ...
    df = get_df("https://finance.yahoo.com/screener/predefined/undervalued_growth_stocks")[0]
...
def get_current_technology_growth_stocks() -> pd.DataFrame:
    """Get data for today's stocks with low PR ratio and growth rate better than 25%"""
    df = get_df("https://finance.yahoo.com/screener/predefined/growth_technology_stocks")[0]
    df.dropna(how="all", axis=1, inplace=True)
    df = df.replace(float("NaN"), "")
    return df
...
def get_current_undervalued_large_cap_stocks() -> pd.DataFrame:
    ...
    df = get_df("https://finance.yahoo.com/screener/predefined/undervalued_large_caps")[0]
    df.dropna(how="all", axis=1, inplace=True)
    df = df.replace(float("NaN"), "")
    return df
...
def get_current_aggressive_small_cap_stocks() -> pd.DataFrame:
    ...
    df = get_df("https://finance.yahoo.com/screener/predefined/aggressive_small_caps")[0]
    df.dropna(how="all", axis=1, inplace=True)
    df = df.replace(float("NaN"), "")
    return df
...
def get_current_stock_price_info(api_key: Dict[str, str], stock_ticker: str) -> Optional[Dict[str, Any]]:
    """
    Return current price information given a stock ticker symbol.
    """
    result = request(f"https://finnhub.io/api/v1/quote?symbol={stock_ticker}&token={api_key['FINNHUB']}")
    if result.status_code != 200:
        return None
    return result.json()


def get_latest_news_for_stock(api_key: Dict[str, str], stock_id: str, limit: int=10) -> List[Dict[str,Any]]:
    ...
    business_sources = ['bloomberg','business-insider','financial-post','fortune','info-money','the-wall-street-journal']
    for source in business_sources:
        assert source in cat_to_id['business']

    start_date = (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d")
    end_date = datetime.now().strftime("%Y-%m-%d")
    articles = newsapi.get_everything(q=stock_id,
...
def get_topk_trending_news(limit: int = 10, extract_content: bool = False) -> List[Dict[str,Any]]:
...
def get_google_trends_for_query(query: str, find_related: bool=False, region: str='') -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        pytrend.build_payload(kw_list=[query], timeframe='today 1-m', geo=region, cat=12)
