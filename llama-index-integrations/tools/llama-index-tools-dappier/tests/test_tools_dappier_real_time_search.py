...
class TestDappierRealTimeSearchTool:
    def test_init_without_api_key_raises_value_error(self, monkeypatch, mocker):
        monkeypatch.delenv("DAPPIER_API_KEY", raising=False)
        dappier_client = mocker.Mock()
        mocker.patch("dappier.Dappier", return_value=dappier_client)
    ...
    def test_search_real_time_data_returns_response_message(
        self, tool, dappier_client, mocker
    ):
