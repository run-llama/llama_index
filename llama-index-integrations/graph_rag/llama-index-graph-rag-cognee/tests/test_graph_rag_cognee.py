...
async def test_empty_documents():
    ...
    # Test empty list
    with pytest.raises(ValueError, match="Invalid data type. Please provide a list of Documents or a single Document."):
        await cogneeRAG.add([])
...
async def test_visualize_graph():
    ...
    with pytest.raises(ValueError, match="is not a directory"):
        await cogneeRAG.visualize_graph(output_file_path="/invalid/path/that/does/not/exist")
...
async def test_mock_full_workflow():
    ...
    with patch('cognee.add', new_callable=AsyncMock) as mock_add, \
         ...
         patch('cognee.modules.users.methods.get_default_user', new_callable=AsyncMock) as mock_user:
