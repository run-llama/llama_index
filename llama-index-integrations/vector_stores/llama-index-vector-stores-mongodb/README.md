...
- **Full-Text / Atlas Search compound filter**: Applied inside the `$search` stage using Atlas Search clause objects (`equals`, `range`, `in`, `must`, `should`, `mustNot`, `minimumShouldMatch`). Generated by `filters_to_search_filter` and passed as `search_filter`.
