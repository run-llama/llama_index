...
from sqlalchemy import MetaData, Table, Column, String, Integer, insert
...
def db_hnsw(conn: Any) -> Generator:
    ...
    yield
    with conn.cursor() as c:
        # encerra conexões antes de apagar o DB
        c.execute(f"""
...
@pytest.fixture()
def pg_hnsw_multiple(db_hnsw: None) -> Generator[List[ParadeDBVectorStore], None, None]:
    """
    This creates multiple instances of ParadeDBVectorStore.
    """
    pgs = []
    for _ in range(2):
        pg = ParadeDBVectorStore.from_params(
...
def db(conn: Any) -> Generator:
    ...
    yield
    with conn.cursor() as c:
        # Força o término de conexões abertas antes de dropar
        c.execute(f"""
            SELECT pg_terminate_backend(pid)
            FROM pg_stat_activity
...
async def test_add_to_db_and_query(
    pg_fixture: ParadeDBVectorStore, node_embeddings: List[TextNode], use_async: bool
) -> None:
    if use_async:
        await pg_fixture.async_add(node_embeddings)
    else:
        pg_fixture.add(node_embeddings)
    assert isinstance(pg_fixture, ParadeDBVectorStore)
...
async def test_query_hnsw(
    pg_hnsw: ParadeDBVectorStore, node_embeddings: List[TextNode], use_async: bool
):
    if use_async:
    ...
    assert isinstance(pg_hnsw, ParadeDBVectorStore)
...
async def test_sparse_query(
    ...
    assert isinstance(pg_hybrid, ParadeDBVectorStore)
...
@pytest.mark.parametrize("use_async", [True, False])
async def test_hybrid_query(
    pg_hnsw_hybrid: ParadeDBVectorStore,
    hybrid_node_embeddings: List[TextNode],
    use_async: bool,
) -> None:
    if use_async:
    ...
    assert isinstance(pg_hnsw_hybrid, ParadeDBVectorStore)
...
def test_hybrid_query_fails_if_no_query_str_provided(
    pg_hybrid: ParadeDBVectorStore, hybrid_node_embeddings: List[TextNode]
) -> None:
    q = VectorStoreQuery(
...
async def test_add_to_db_and_query_index_nodes(
    pg_fixture: ParadeDBVectorStore,
...
async def test_delete_nodes(
    pg_fixture: ParadeDBVectorStore, node_embeddings: List[BaseNode], use_async: bool
) -> None:
    if use_async:
        await pg_fixture.async_add(node_embeddings)
    else:
...
async def test_hnsw_index_creation(
    pg_hnsw_multiple: List[ParadeDBVectorStore],
    ...
    # these are the actual table and index names that ParadeDBVectorStore automatically created
...
async def test_custom_engines(db: None, node_embeddings: List[TextNode]) -> None:
    """Test that ParadeDBVectorStore works correctly with custom engines."""
    from sqlalchemy import create_engine
    ...
    # Create ParadeDBVectorStore with custom engines
    pg = ParadeDBVectorStore(
        embed_dim=TEST_EMBED_DIM,
        engine=engine,
...
def test_custom_sync_engine_only(db: None, node_embeddings: List[TextNode]) -> None:
    """Test that ParadeDBVectorStore works correctly with only a custom sync engine."""
    from sqlalchemy import create_engine
    ...
    # Create ParadeDBVectorStore with custom sync engine only
    with pytest.raises(ValueError):
        _ = ParadeDBVectorStore(
            embed_dim=TEST_EMBED_DIM,
            engine=engine,
        )
...
async def test_custom_async_engine_only(
    ...
    """Test that ParadeDBVectorStore works correctly with only a custom async engine."""
    from sqlalchemy.ext.asyncio import create_async_engine
    ...
    # Create ParadeDBVectorStore with custom async engine only
    with pytest.raises(ValueError):
        _ = ParadeDBVectorStore(
            embed_dim=TEST_EMBED_DIM,
            async_engine=async_engine,
...
async def test_indexed_metadata(
    pg_indexed_metadata: ParadeDBVectorStore,
    hybrid_node_embeddings: List[TextNode],
) -> None:
...
async def test_bm25_sparse_query(
    ...
    assert res.nodes
    assert len(res.nodes) == 2
    # BM25 should rank documents with "fox" highest - both ccc and ddd have "fox"
...
async def test_bm25_hybrid_query(
    ...
    assert res.nodes
    assert len(res.nodes) >= 2  # At least from vector search
    # Should contain results from both vector and BM25 search
    node_ids = [node.node_id for node in res.nodes]
    # Either ccc or ddd should be in results (both have "fox")
    assert "ccc" in node_ids or "ddd" in node_ids
...
@pytest.mark.asyncio
@pytest.mark.parametrize("use_async", [True, False])
async def test_bm25_with_metadata_filters(
    pg_bm25: ParadeDBVectorStore,
    hybrid_node_embeddings: List[TextNode],
    use_async: bool,
) -> None:
    ...
    # Note: Metadata filters with BM25 raw SQL may not work as expected
...
async def test_bm25_hnsw_hybrid(
    ...
    assert res.nodes
    assert len(res.nodes) >= 2
    # Verify BM25 results are included
    node_ids = [node.node_id for node in res.nodes]
    assert "ccc" in node_ids or "ddd" in node_ids
...
async def test_bm25_index_creation(
    ...
    pg_bm25.add(node_embeddings[:1])  # Add at least one node to trigger index creation
...
async def test_bm25_empty_query(
    ...
    # Test with empty query string - BM25 should raise an error
    q = VectorStoreQuery(
        query_embedding=_get_sample_vector(0.1),
        query_str="   ",
        sparse_top_k=2,
    ...
    # BM25 will fail with empty queries, expect an error
    with pytest.raises(Exception):  # Can be InternalError or other
        if use_async:
