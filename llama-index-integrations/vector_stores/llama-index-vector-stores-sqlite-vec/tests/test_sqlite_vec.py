...
def test_sqlite_vec_installed():
    assert importlib.util.find_spec("sqlite_vec") is not None
...
def test_instance_creation_from_connection(
    text_node_list: List[TextNode],
) -> None:
    conn = sqlite3.connect(":memory:")
    vector_store = SqliteVecVectorStore(connection=conn, embed_dim=3)
...
def test_from_local():
    store = SqliteVecVectorStore.from_local(database_path=":memory:", embed_dim=3)
    assert isinstance(store, SqliteVecVectorStore)


def memory_store():
    return SqliteVecVectorStore(embed_dim=3)


def disk_store():
    db_path = "./storage/test_sqlite_vec.db"
    if os.path.exists(db_path):
        os.remove(db_path)
    return SqliteVecVectorStore(database_path=db_path, embed_dim=3)


@pytest.mark.parametrize("persistent", ["memory", "disk"])
class TestStore:
    @pytest.fixture
    def vector_store(self, persistent: str) -> SqliteVecVectorStore:
        if persistent == "memory":
            return memory_store()
        return disk_store()

    def test_add_and_query(
        self, vector_store: SqliteVecVectorStore, text_node_list: List[TextNode]
    ) -> None:
        vector_store.add(text_node_list)
    ...
    def test_get_nodes(
        ...
        assert all(n.node_id in [t.node_id for t in text_node_list] for n in nodes)
