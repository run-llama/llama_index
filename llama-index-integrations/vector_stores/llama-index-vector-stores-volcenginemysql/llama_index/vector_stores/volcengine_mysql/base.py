...
# ----------------------------------------------------------------------
# Minimal example usage (for reference only)
# ----------------------------------------------------------------------


def example_usage() -> None:
    """
    A minimal end-to-end example of using
    :class:`VolcengineMySQLVectorStore`.

    This example demonstrates how to:

    - Initialize :class:`VolcengineMySQLVectorStore` via
      :meth:`from_params`.
    - Inject it into a LlamaIndex :class:`StorageContext`.
    - Build a :class:`VectorStoreIndex` and execute a simple query.

    The code is intended purely as a structural reference. Before
    running it in your environment you should:

    - Replace ``host``/``user``/``password``/``database``/
      ``table_name`` with your actual RDS MySQL settings.
    - Ensure that the instance has vector indexing enabled and meets
      the required version.
    - Install the required dependencies: ``sqlalchemy``, ``pymysql``,
      and ``llama-index``.
    """
    from llama_index.core import Document, StorageContext, VectorStoreIndex

    # 1. Initialize the vector store (assuming an embedding model is
    #    already configured at the application level)
    vector_store = VolcengineMySQLVectorStore.from_params(
        host="your-mysql-host",  # Replace with your actual RDS MySQL host
        port=3306,
        user="your_user",
        password="your_password",
        database="your_database",
        table_name="llama_index_vectorstore",
        embed_dim=1536,  # Must match the upstream embedding model dimension
    ...
    # 2. Build a StorageContext and create a VectorStoreIndex backed by
    #    the vector store
    storage_context = StorageContext.from_defaults(vector_store=vector_store)

    documents = [
        Document(
            text="Volcengine Cloud Database for MySQL provides native vector index capabilities."
        ),
        Document(
            text="LlamaIndex is a framework for building LLM applications over private or domain-specific data."
        ),
    ]
    ...
    response = query_engine.query(
        "Does Volcengine RDS for MySQL support vector search?"
    )
    print(response)


if __name__ == "__main__":  # pragma: no cover - manual demo entry
    # Demo entry point only; do not use this pattern in production.
    example_usage()
