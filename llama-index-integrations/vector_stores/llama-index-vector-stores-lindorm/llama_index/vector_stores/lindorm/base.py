...
class LindormVectorClient:
    ...
    Optional Keyword Args for HNSW:
        m(int): maximum number of outgoing edges in each layer of the graph. Between 1 and 100; defualt: 16
        ef_construction(int): Length of the dynamic list when the index is built. Between 1 and 1000; defualt: 100        
    ...
    def _knn_search_query(
        ...
        if not filters:
            search_query = self._default_approximate_search_query(
                query_vector=query_embedding, k=k, vector_field=vector_field,nprobe=nprobe, reorder_factor=reorder_factor
            )
        else:
