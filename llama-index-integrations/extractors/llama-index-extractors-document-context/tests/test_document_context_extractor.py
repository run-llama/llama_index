    ...
    def _create_nodes(document, texts):
        ...
        return [
            TextNode(
                ...
                metadata={},
                relationships={
                    NodeRelationship.SOURCE: doc_info
                }
            ) for text in texts
        ]
...
@pytest.mark.asyncio
async def test_context_extraction_basic(context_extractor, sample_documents, create_text_nodes):
    doc = sample_documents[0]
    nodes = create_text_nodes(doc, [
        "This is chapter 1.",
...
@pytest.mark.asyncio
async def test_context_extraction_custom_prompt(docstore, mock_llm, sample_documents, create_text_nodes):
...
@pytest.mark.asyncio
async def test_multiple_documents_context(context_extractor, sample_documents, create_text_nodes):
    # Create nodes from different documents
    nodes = (
        create_text_nodes(sample_documents[0], ["This is chapter 1."]) +
        create_text_nodes(sample_documents[1], ["Chapter 2 builds on previous concepts."])
    )
