...
class IOHandler(BaseIOHandler):
    ...
    def deserialize_output(self, response: "StreamingBody") -> List[List[float]]:
                ...
                # Ensure all embedding values are native Python floats to avoid
                # pydantic validation errors when SageMaker returns numpy types or ints.
                return [[float(x) for x in embedding] for embedding in result]
