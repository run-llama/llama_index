...
class HubPlanner:
    def __init__(
        ...
        self.template_planner = self.template_planner.partial_format(output_format=planner_output_format, output_format_empty=planner_output_empty_format)


        lc_output_parser = JsonOutputParser()
        self.output_parser = LangchainOutputParser(lc_output_parser)

        self.query_engine = QueryPipeline(chain=[self.template_planner, self.llm, self.output_parser], verbose=True) 

    # Generate a plan based on the user's query
    def plan_generate(self, query, tool_info, chat_history):      
           
        plan = self.query_engine.run(input=query, tools=tool_info, chat_history=chat_history)
        return plan
