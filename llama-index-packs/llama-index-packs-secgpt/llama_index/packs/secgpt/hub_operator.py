...
class HubOperator:
    ...
    def __init__(self, tool_importer, user_id):
    ...
    def filter_plan(self, plan):
        ...
        self.app_data_dict = {step['name']: step['input'] for step_list in self.plan for step in step_list}
    ...
    def execute_app_spoke(self, query, shared_data_dict, requested_app, flag=False):    
        ...
        if requested_app in self.spoke_tool.keys():
            ...
            if shared_data_dict:
                action_message = f'Your data "{shared_data_dict}" is sharing with "{requested_app}"'
            ...
            p = multiprocessing.Process(target=spoke.run_process, args=(child_sock, query, spoke_session_id, shared_data))
            p.start()
            ...
            print("Using " + requested_app + " spoke ...\n")
            tool = [t for t in self.tools if t.metadata.name == requested_app][0]

            collab_functionalities = self.tool_importer.get_collab_functions(requested_app)
        ...
        while True:
            ...
            if data['message_type'] == 'app_request':
                ...
                if functionality_request not in self.function_tools.keys():
                    ...
                    shared_data_dict = None
                    if tool in self.app_data_dict.keys():
                    ...
                    app_response = self.execute_app_spoke(str(data), shared_data_dict, tool, False)
