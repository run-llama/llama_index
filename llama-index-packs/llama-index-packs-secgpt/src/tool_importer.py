...
class ToolImporter:
    ...
    def __init__(self, tools, tool_specs=[]):
        ...
        for tool_spec in tool_specs:
            ...
            self.tool_functions[tool_spec.__class__.__name__] = list(spec_tool_name_obj_dict.keys())
    ...
    def get_collab_functions(self, tool_name=None):
        if tool_name:
            return [tool.metadata.name for tool in self.tools if tool.metadata.name != tool_name]
        else:
            return [tool.metadata.name for tool in self.tools]
...
def create_message_spoke_tool():
    ...
    tool_message_spoke = FunctionTool.from_defaults(
        fn=message_spoke,
        name="message_spoke",
