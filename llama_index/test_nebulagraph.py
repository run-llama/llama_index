...
def test_hash_string_to_rank():
    """Test the hash_string_to_rank function."""
    # Test with a known string and its hash value
    assert hash_string_to_rank("test") == 2427306421058692164

    # Test with another known string and its hash value
    assert hash_string_to_rank("another test") == 8748319316464553227

    # Test with an empty string, should return 0
    assert hash_string_to_rank("") == 0
    # get signed 64-bit hash value

    signed_hash = hash("some string")
...
def test_prepare_subjs_param():
    """Test the prepare_subjs_param function."""
    from nebula3.common import ttypes

    # Test with a list of strings
    subjs = ["test1", "test2", "test3"]
    result = prepare_subjs_param(subjs)
    assert isinstance(result, dict)
    assert "subjs" in result
    assert isinstance(result["subjs"], ttypes.Value)
    assert result["subjs"].lVal.values[0].get_sVal() == "test1"
    assert result["subjs"].lVal.values[1].get_sVal() == "test2"
    assert result["subjs"].lVal.values[2].get_sVal() == "test3"

    # Test with None
    result = prepare_subjs_param(None)
    assert isinstance(result, dict)
    assert not result

    # Test with an empty list
    result = prepare_subjs_param([])
    assert isinstance(result, dict)
    assert not result

    # Test with a list containing non-string elements
    with pytest.raises(TypeError):
        prepare_subjs_param([1, 2, 3])
...
def test_escape_str():
    """Test the escape_str function."""

    # Test with a string containing double quotes
    result = escape_str('This is a "test" string.')
    ...
    # Test with a string containing no double quotes or spaces
    result = escape_str('This is a test string.')
    assert result == 'This is a test string.'
...
def test_nebula_graph_store():
    """Test the NebulaGraphStore class."""

    # Test __init__ function
    nebula_graph_store = NebulaGraphStore(space_name="test_space")
    assert nebula_graph_store._space_name == "test_space"

    # Test init_session_pool function
    session_pool = nebula_graph_store.init_session_pool()
    assert session_pool is not None

    # Test execute function
    query = "SHOW TAGS"
    result = nebula_graph_store.execute(query)
    assert result is not None

    # Test from_dict function
    config_dict = {"space_name": "test_space"}
    nebula_graph_store = NebulaGraphStore.from_dict(config_dict)
    assert nebula_graph_store._space_name == "test_space"
    ...
    config_dict = nebula_graph_store.config_dict
    assert config_dict["space_name"] == "test_space"

    # Test get function
    triplets = nebula_graph_store.get("test_subj")
    assert isinstance(triplets, list)

    # Test get_flat_rel_map function
    rel_map = nebula_graph_store.get_flat_rel_map(["test_subj"])
    assert isinstance(rel_map, dict)

    # Test get_rel_map function
    rel_map = nebula_graph_store.get_rel_map(["test_subj"])
    assert isinstance(rel_map, dict)

    # Test upsert_triplet function
    nebula_graph_store.upsert_triplet("test_subj", "test_rel", "test_obj")

    # Test delete function
    nebula_graph_store.delete("test_subj", "test_rel", "test_obj")

    # Test refresh_schema function
    nebula_graph_store.refresh_schema()

    # Test get_schema function
    schema = nebula_graph_store.get_schema()
    assert isinstance(schema, str)

    # Test query function
    result = nebula_graph_store.query("SHOW TAGS")
    assert isinstance(result, dict)


if __name__ == "__main__":
    test_nebula_graph_store()
