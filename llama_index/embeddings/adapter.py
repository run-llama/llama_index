...
class LinearAdapterEmbeddingModel(BaseEmbedding):
    ...
    def _get_query_embedding(self, query: str) -> List[float]:
        ...
        if self._transform_query:
            ...
            query_embedding_t = self._adapter.forward_transpose(query_embedding_t)
    ...
    def _get_text_embedding(self, text: str) -> List[float]:
        ...
        if not self._transform_query:
            text_embedding_t = torch.tensor(text_embedding).to(self._target_device)
            text_embedding_t = self._adapter(text_embedding_t)
            text_embedding = text_embedding_t.tolist()
