...
class GPTIndexMemory(Memory):
    ...
    Args:
        ...
        input_keys (Optional[List[str]]): Input keys. Defaults to None.
    ...
    def _get_prompt_input_keys(self, inputs: Dict[str, Any]) -> List[str]:
        if self.input_keys is None:
            prompt_input_keys = list(set(inputs).difference([self.memory_key, "stop"]))
        else:
            prompt_input_keys = self.input_keys
        return prompt_input_keys

    def load_memory_variables(self, inputs: Dict[str, Any]) -> Dict[str, str]:
        """Return key-value pairs given the text input to the chain."""
        prompt_input_keys = self._get_prompt_input_keys(inputs)
        response_dict = {}
        for key in prompt_input_keys:
            query_str = inputs[key]
            query_engine = self.index.as_query_engine(**self.query_kwargs)
            response = query_engine.query(query_str)
            response_dict[key] = str(response)
    ...
    def save_context(self, inputs: Dict[str, Any], outputs: Dict[str, str]) -> None:
        ...
        for key in prompt_input_keys:
            if self.output_key is None:
                if key in outputs and len(outputs) == 1:
                    output_key = key
                else:
                    raise ValueError(f"One output key expected for input key '{key}'")
            else:
                output_key = self.output_key

            human = f"{self.human_prefix}: " + inputs[key]
...
class GPTIndexChatMemory(BaseChatMemory):
    ...
    def load_memory_variables(self, inputs: Dict[str, Any]) -> Dict[str, str]:
        """Return key-value pairs given the text input to the chain."""
        response_dict = {}
        for key in self._get_prompt_input_keys(inputs):
            query_str = inputs[key]
            query_engine = self.index.as_query_engine(**self.query_kwargs)
    ...
    def save_context(self, inputs: Dict[str, Any], outputs: Dict[str, str]) -> None:
        ...
        for key in self._get_prompt_input_keys(inputs):
            ...
            human_message = HumanMessage(content=inputs[key])
            ...
            human_txt = f"{self.human_prefix}: " + inputs[key]
