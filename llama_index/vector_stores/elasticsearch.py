...
async def _aget_elasticsearch_client(
    *,
    es_url: Optional[str] = None,
    cloud_id: Optional[str] = None,
    api_key: Optional[str] = None,
    ...
    es_client = elasticsearch.AsyncElasticsearch(**connection_params)
    try:
        await es_client.info()
    except Exception as e:
        logger.error(f"Error connecting to AsyncElasticsearch: {e}")
        raise e
...
class AsyncElasticsearchStore(VectorStore):
    """Asyncrhonious Elasticsearch vector store.
    ...
    async def create(
        ...
        if es_client is not None:
            ...
            self._client = await _aget_elasticsearch_client(
                es_url=es_url,
                username=es_user,
                password=es_password,
                cloud_id=es_cloud_id,
    ...
    async def add(
        ...
        *,
        ...
        """Async method to add nodes to AsyncElasticsearch index.
    ...
    async def delete(self, ref_doc_id: str, **delete_kwargs: Any) -> None:
        """Async delete node from AsyncElasticsearch index.
