...
class RetryStrategy(ABC):
    ...
    def decorate(self, func: Callable) -> Callable:
        """
        Given a function, return a function that wraps the given function with
        retry logic.
...
class ExponentialBackoffRetryStrategy(RetryStrategy):
    """
    Retry strategy that retries a function with exponential backoff when it
    raises an exception.
    ...
    def __init__(
        ...
        if num_attempts < 1:
            raise ValueError(
                "num_attempts must be greater than or equal to 1. "
                f"num_attempts={num_attempts}"
            )
        self._num_attempts = num_attempts
        if min_wait > max_wait:  # type: ignore
            raise ValueError(
                "min_wait must be less than or equal to max_wait. "
