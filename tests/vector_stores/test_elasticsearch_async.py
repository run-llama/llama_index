...
from llama_index.vector_stores import AsyncElasticsearchStore
...
async def test_instance_creation(
    ...
    es_store = await AsyncElasticsearchStore.create(
        **elasticsearch_connection,
        index_name=index_name,
    )
    assert isinstance(es_store, AsyncElasticsearchStore)
...
async def test_add_to_es_and_query(
    ...
    await es_store.add(node_embeddings)
    res = await es_store.query(
