...
from llama_index.schema import Document, MetadataMode, NodeRelationship, ObjectType
...
def test_get_nodes_from_document_with_node_relationship(
    ...
    assert (
        nodes[0].relationships[NodeRelationship.SOURCE].node_type == ObjectType.DOCUMENT
    )
    assert nodes[0].relationships[NodeRelationship.NEXT].node_type == ObjectType.TEXT
