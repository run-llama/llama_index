...
@pytest.fixture()
def cosmosdb_docstore(mongo_kvstore: MongoDBKVStore) -> CosmosDBDocStore:
    return CosmosDBDocStore(mongo_kvstore=mongo_kvstore)


@pytest.mark.skipif(MongoClient is None, reason="pymongo not installed")
def test_cosmosdb_docstore(
    cosmosdb_docstore: CosmosDBDocStore, documents: List[Document]
) -> None:
    ds = cosmosdb_docstore
...
@pytest.mark.skipif(MongoClient is None, reason="pymongo not installed")
def test_cosmosdb_docstore_hash(
    cosmosdb_docstore: CosmosDBDocStore, documents: List[Document]
) -> None:
    ds = cosmosdb_docstore
