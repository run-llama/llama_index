...
def test_paragraphs() -> None:
    ...
    sentence_text_splitter = SentenceAwareNodeParser(chunk_size=20, chunk_overlap=0)
...
def test_chinese_text(chinese_text: str) -> None:
    splitter = SentenceAwareNodeParser(chunk_size=512, chunk_overlap=0)
    chunks = splitter.split_text(chinese_text)
    assert len(chunks) == 2


def test_contiguous_text(contiguous_text: str) -> None:
    splitter = SentenceAwareNodeParser(chunk_size=100, chunk_overlap=0)
...
def test_edge_case() -> None:
    ...
    splitter = SentenceAwareNodeParser(tokenizer=tiktoken.get_encoding("gpt2").encode)
...
def test_overlap() -> None:
    splitter = SentenceAwareNodeParser(chunk_size=15, chunk_overlap=10)
