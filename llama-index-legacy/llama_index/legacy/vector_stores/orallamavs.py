...
from llama_index.core.schema import BaseNode, TextNode, NodeRelationship, RelatedNodeInfo, MetadataMode
...
def _handle_exceptions(func):
  ...
  def wrapper(*args, **kwargs):
    try:
      ...
      logger.exception("GPU-related error occurred.")
...
class OraLlamaVS(VectorStore):
  ...
  def _append_meta_filter_condition(self,
    where_str: Optional[str], exact_match_filter: list
    ) -> str:
      filter_str = " AND ".join(
  ...
  def _build_insert( self, values: List[BaseNode] ) -> (str, List[tuple]):
    ...
    for item in values:
      item_values = tuple(
        column['extract_func'](item)
        for column in column_config.values()
      )
  ...
  def query(self,
    ...
    with self._client.cursor() as cursor:
      ...
      for result in results:
        ...
        node_info = json.loads(result[3]) if isinstance(result[3], str) else result[3]
        metadata = json.loads(result[4]) if isinstance(result[4], str) else result[4]
