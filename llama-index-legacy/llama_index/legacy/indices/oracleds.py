                ...
                try:
                    ...
                    if self.mdata_cols is not None:
                        ...
                        sql = "select column_name, data_type from all_tab_columns where owner = '" + \
                            self.owner.upper() + "' and " +"table_name = '" + self.tablename.upper() + "'"
                    ...
                    for row in cursor:
                        ...
                        if row is None:
                            ...
                            if row[1] is None:
                                ...
                                results.append(Document(text=str(row[1]), metadata=metadata))
                except Exception as ex:
                    print(f"An exception occurred :: {ex}")
                    traceback.print_exc()
                    cursor.close()
    ...
    def _get_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            cursor.execute("select t.* from dbms_vector_chain.utl_to_embeddings(:content, json(:params)) t",
                content=text, params=json.dumps(self._params))
            ...
            if isinstance(docs, str):
                ...
                cursor.execute("""
                    ...
                    begin
                        input := :data;
                        :summ := dbms_vector_chain.utl_to_summary(input, json(:params));
                    end;""", data=docs.text, params=json.dumps(self.summary_params), summ=summary)
...
class OracleTextSplitter():
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        try:
            ...
            cursor.execute("select t.* from dbms_vector_chain.utl_to_chunks(:content, json(:params)) t",
