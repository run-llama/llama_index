"""Prepare the new-docs folder for building the documentation.
...
MKDOCS_YML = "new-docs/mkdocs.yml"

# examples config
EXAMPLES_DIR = "new-docs/docs/examples"
    ...
    for file_name in os.listdir(path_name):
        if file_name.endswith(".ipynb"):
            toc_path_name = "./" + os.path.join(
                path_name.replace("new-docs/docs/", ""), file_name
            )
            if (
                toc_path_name
        ...
        if os.path.isdir(os.path.join(path_name, file_name)):
            for root, dirs, files in os.walk(
                ...
                for file in files:
                    if file.endswith(".ipynb"):
                        toc_path_name = "./" + os.path.join(
                            root.replace("new-docs/docs/", ""), file
                        )
                        if (
                            toc_path_name
        ...
        for file in files:
            if file == "pyproject.toml":
                ...
                full_path = os.path.join(
                    "new-docs/docs/api_reference", folder_name
                )
...
os.system("cp CHANGELOG.md new-docs/docs/CHANGELOG.md")
os.system("cp CONTRIBUTING.md new-docs/docs/CONTRIBUTING.md")
os.system("cp new-docs/DOCS_README.md new-docs/docs/DOCS_README.md")
