...
async def test_chat_prompt_helper_atruncate_multimodal(
    ...
    assert truncated_messages2_strict == [
        ...
        # Second message should be truncated to just the audio block
        ChatMessage(blocks=[ab]),
        # Third message should be truncated to just the image block, which happens to be smaller than
        # tb.estimate_tokens() + ab.estimate_tokens() = 262 which is greater than ib.estimate_tokens() = 258
        ChatMessage(blocks=[ib]),
        # The remaining messages are truncated to be empty so strict truncation results in no messages
    ]
...
async def test_chat_prompt_helper_chat_prompt_template_repack_arbitrary(
    ...
    # Splits into three messages due to space constraints
    ...
    # 3 prompts due to space constraints
    # 1 with the System message + 1 user message with text and audio blocks,
    # 1 with the System message + 1 user message with text and video block,
    # 1 with the System message + 1 user message with document block
    assert len(formatted_prompts2) == 3
    ...
    # Second prompt should contain prompt text + video block
    ...
    # Third prompt should contain prompt text + document block
    assert formatted_prompts2[2][1].role == "user"
    assert formatted_prompts2[2][1].blocks[0] == TextBlock(
        text="Describe the following content."
    )
    assert formatted_prompts2[2][1].blocks[1] != db
    assert (
        formatted_prompts2[2][1].blocks[1].resolve_document().read()
        == db.resolve_document().read()
    )
