...
def _query_ids(
    store: SimpleVectorStore, embedding: List[float], top_k: int
) -> List[str]:
    result = store.query(
...
def test_query_reflects_mutations_after_a_prior_query() -> None:
    """
    A query must never be served from state cached before a mutation.

    SimpleVectorStore.query rebuilds its embedding matrix on every call, so any
    caching added for speed has to be invalidated by add, delete and clear. The
    first query is what populates such a cache, so each mutation below is
    followed by a query that would expose a stale one.
    """
    store = SimpleVectorStore()
    store.add(
        [
            TextNode(text="a", id_="a", embedding=[1.0, 0.0]),
            TextNode(text="b", id_="b", embedding=[0.0, 1.0]),
        ]
    )
    query = [1.0, 0.0]

    assert _query_ids(store, query, 2) == ["a", "b"]

    store.add([TextNode(text="c", id_="c", embedding=[0.9, 0.1])])
    assert set(_query_ids(store, query, 3)) == {"a", "b", "c"}

    store.delete_nodes(node_ids=["a"])
    assert set(_query_ids(store, query, 3)) == {"b", "c"}

    store.clear()
    assert _query_ids(store, query, 3) == []

    store.add([TextNode(text="d", id_="d", embedding=[0.5, 0.5])])
    assert _query_ids(store, query, 3) == ["d"]


def test_query_reflects_upsert_of_an_existing_node_id() -> None:
    """
    Re-adding an existing id with a new embedding leaves the id set and the node
    count unchanged, so a cache keyed on either one goes stale without this.
    """
    store = SimpleVectorStore()
    store.add(
        [
    ...
    assert _query_ids(store, query, 1) == ["b"]

    # same id, now pointing the other way; "a" must win the next query
    store.add([TextNode(text="a", id_="a", embedding=[0.0, 1.0])])
    store.add([TextNode(text="b", id_="b", embedding=[1.0, 0.0])])

    assert _query_ids(store, query, 1) == ["a"]
