...
@pytest.mark.asyncio
async def test_state_prompt_refreshes_after_tool_updates_state():
    """The state prompt follows the state, and is rendered exactly once per step."""
    from llama_index.core.workflow import Context
    ...
    # the first step sees the initial state, the second one the state the tool wrote
    assert "'counter': 0" in captured_user_messages[0]
    assert "'counter': 1" in captured_user_messages[1]
    # and the prompt is rendered once per step, never nested into itself
    for message in captured_user_messages:
        assert message.count("Current state:") == 1
        assert message.endswith("User message: test")

    # the stored conversation still holds what the user actually wrote
    stored = await memory.aget()
    assert str(stored[0].content) == "test"


@pytest.mark.asyncio
async def test_state_prompt_does_not_rewrite_memory_without_tool_calls():
    """A single-step run leaves the stored user message alone as well."""
    agent = FunctionAgent(
        name="agent",
        description="test",
        llm=MockFunctionCallingLLM(
