  ...
  class MockSQLQueryEngine(BaseSQLTableQueryEngine):
      ...
      def query(self, q): return Response(response="SQL Response", metadata={"sql_query": "SELECT *"})
      async def aquery(self, q): return Response(response="SQL Response", metadata={"sql_query": "SELECT *"})
  
  @pytest.fixture
  def mock_sql_tool():
      qe = MagicMock(spec=BaseSQLTableQueryEngine)
      qe.query.return_value = Response(response="SQL Response", metadata={"sql_query": "SELECT *"})
      qe.aquery = AsyncMock(return_value=Response(response="SQL Response", metadata={"sql_query": "SELECT *"}))
      return QueryEngineTool.from_defaults(query_engine=qe, name="sql_tool", description="SQL Tool")
  ...
  def mock_other_tool():
      ...
      return QueryEngineTool.from_defaults(query_engine=qe, name="other_tool", description="Other Tool")
  
  @pytest.fixture
  def mock_info_selector(): return MagicMock(spec=LLMSingleSelector)
  ...
  def test_sql_join_query_engine_sync(mock_sql_tool, mock_other_tool, mock_llm):
      ...
      e = SQLJoinQueryEngine(sql_query_tool=mock_sql_tool, other_query_tool=mock_other_tool, selector=s, llm=mock_llm, use_sql_join_synthesis=False)
