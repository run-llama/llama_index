...
def _mixed_content_for_node_embed_call(
    node: BaseNode, metadata_mode: MetadataMode
) -> MixedEmbeddingContent:
    """Content for embed_batch: mixed blocks if present, else a single text block."""
    from llama_index.core.base.llms.types import TextBlock

    mixed = node.get_mixed_embedding_content(metadata_mode=metadata_mode)
    if mixed is not None:
        return mixed
    return [TextBlock(text=node.get_content(metadata_mode=metadata_mode))]
...
class BaseEmbedding(TransformComponent, DispatcherSpanMixin):
    ...
    def supports_mixed_embedding(self) -> bool:
        ...
        When any node in a batch has mixed content, the whole batch uses
        embed_batch; if this is False, __call__ / acall raise instead of
    ...
    def __call__(self, nodes: Sequence[BaseNode], **kwargs: Any) -> Sequence[BaseNode]:
        ...
        any_mixed = any(
            node.get_mixed_embedding_content(metadata_mode=MetadataMode.EMBED)
            is not None
            for node in nodes
        )
        if any_mixed:
            if not self.supports_mixed_embedding:
                raise ValueError(
                    "One or more nodes have mixed (multimodal) content, but this "
                    "embedding model does not support mixed embedding "
                    "(supports_mixed_embedding is False). Use a model that sets "
                    "supports_mixed_embedding=True or split mixed and text-only nodes."
                )
            contents = [
                _mixed_content_for_node_embed_call(node, MetadataMode.EMBED)
                for node in nodes
            ]
            embeddings = self.embed_batch(
                contents, show_progress=show_progress, **kwargs
            )
        else:
    ...
    async def acall(
        ...
        if any_mixed:
            ...
            embeddings = await self.aembed_batch(
                contents, show_progress=show_progress, **kwargs
            )
        else:
