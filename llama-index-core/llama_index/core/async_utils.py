...
def run_async_tasks(
    ...
    if show_progress:
        # Only fall back when tqdm.asyncio is unavailable — not on task failures.
        # A broad `except Exception` here would silently swallow task exceptions
        # and then fail again (misleadingly) when trying to re-await consumed
        # coroutines in the fallback path.
        try:
            import nest_asyncio
            ...
            async def _tqdm_gather() -> List[Any]:
                return await _tqdm_cls.gather(*tasks_to_execute, desc=progress_bar_desc)
