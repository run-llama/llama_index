...
class Refine(BaseSynthesizer):
    ...
    def get_response(
        ...
        **response_kwargs: Any,
        ...
        dispatcher.event(
            GetResponseStartEvent(query_str=query_str, text_chunks=list(text_chunks))
        )
        max_prompt = get_biggest_prompt([self._text_qa_template, self._refine_template])
        ...
        while text_chunks_deque:
            if isinstance(response, Generator):
                response = get_response_text(response)

            text_chunk = text_chunks_deque.popleft()

            if response is None:
                prompt_template = self._text_qa_template.partial_format(
                    query_str=query_str
                )
            else:
                prompt_template = self._refine_template.partial_format(
                    query_str=query_str, existing_answer=response
                )
                # Because the existing answer portion is constantly being updated, it may be necessary to
                # repack the text chunk with the new prompt template to ensure it fits. Since the template has been
                # partially formatted with the actual response, there's no need for the extra padding.
                text_chunks = self._prompt_helper.repack(
                    prompt_template, [text_chunk], llm=self._llm
                )
                # If text chunk is too big to be packed into a single chunk, push new chunks into the front of the deque
                if len(text_chunks) > 1:
                    text_chunks_deque.extendleft(text_chunks)
                    continue
...
class MultimodalRefine(BaseMultimodalSynthesizer):
    """Refine a response to a query across text chunks."""
    ...
    def _get_prompts(self) -> PromptDictType:
        ...
        return {
            ...
            "chat_contnet_refine_template": self._chat_content_refine_template,
        }
    ...
    def get_response(
        ...
        **response_kwargs: Any,
        ...
        max_prompt = get_biggest_prompt(
            [self._chat_content_qa_template, self._chat_content_refine_template]
        )
        message_chunks_deque = deque(
            [
                mc
                for message_chunk in message_chunks
                for mc in self._prompt_helper.repack(
                    max_prompt,
                    [message_chunk],  # type: ignore[list-item]
                    llm=self._llm,
                    padding=self._response_padding_size,
                )
            ]
        ...
        response = prev_response
        while message_chunks_deque:
            if isinstance(response, Generator):
                response = get_response_text(response)

            message_chunk = message_chunks_deque.popleft()

            if response is None:
                prompt_template = self._chat_content_qa_template.partial_format(
                    query_str=query_str
                )
            else:
                prompt_template = self._chat_content_refine_template.partial_format(
                    query_str=query_str, existing_answer=response
                )
                # Because the existing answer portion is constantly being updated, it may be necessary to
                # repack the text chunk with the new prompt template to ensure it fits. Since the template has been
                # partially formatted with the actual response, there's no need for the extra padding.
                repacked_chunks = self._prompt_helper.repack(
                    prompt_template, [message_chunk], llm=self._llm
                )
                if len(repacked_chunks) > 1:
                    message_chunks_deque.extendleft(repacked_chunks)
                    continue
                message_chunk = repacked_chunks[0]

            program = self._program_factory(prompt_template)
            prompt_kwargs = {"context_messages": [message_chunk]}
            if resp := self._update_response(program, prompt_kwargs, response_kwargs):
                response = resp
        if isinstance(response, str):
            if self._output_cls is not None:
            ...
            response = cast(Generator, response)
        dispatcher.event(GetMessageResponseEndEvent())
        return response
