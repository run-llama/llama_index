...
from llama_index.core.node_parser.text.utils import truncate_text, truncate_messages
...
class ChatPromptHelper(BaseComponent):
    ...
    def get_message_splitter_given_prompt(
        self,
        prompt: BasePromptTemplate,
        num_chunks: int = 1,
        padding: int = DEFAULT_PADDING,
    ...
    ) -> TokenMessageSplitter:  # type: ignore[return type mismatch]
        """
        Get message splitter configured to maximally pack available context window,
        taking into account of given prompt, and desired number of chunks.
        """
        chunk_size = self._get_available_chunk_size(
            prompt, num_chunks, padding=padding, llm=llm, tools=tools
    ...
    def truncate(
        ...
        """Truncate text chunks to fit available context window."""
        message_splitter = self.get_message_splitter_given_prompt(
            prompt,
        ...
        return truncate_messages(list(messages), message_splitter)
    ...
    def repack(
        ...
        that more fully "pack" the prompt template given the context_window.
        """
        splitter = self.get_message_splitter_given_prompt(
            prompt, padding=padding, llm=llm, tools=tools
        )
        combined_message = ChatMessage(
            blocks=[block for message in messages for block in message.blocks]
        )
        return splitter.split_message(combined_message)
