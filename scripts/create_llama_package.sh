...
# copy files from llama_index dir
cp -r $GPT_INDEX_DIR/llama_index $LLAMA_INDEX_DIR/llama_index
...
cd $LLAMA_INDEX_DIR
# replace all usages of llama_index with llama_index
find llama_index/. -type f -name '*.py' -print0 | xargs -0 sed -i '' "s/llama_index/llama_index/g"
# build package
