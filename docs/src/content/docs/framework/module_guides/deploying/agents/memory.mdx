...
blocks = [
    ...
    VectorMemoryBlock(
        name="vector_memory",
        # required: pass in a vector store like qdrant, chroma, weaviate,
        # milvus, Azure Cosmos DB for NoSQL, etc.
...
`VectorMemoryBlock` uses the generic vector store interface, so an `AzureCosmosDBNoSqlVectorSearch` from the [Azure Cosmos DB for NoSQL vector store quickstart](/python/framework/integrations/vector_stores/azurecosmosdbnosqldemo) can be passed as `vector_store`.
