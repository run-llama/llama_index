...
When building chat-based multi-modal applications, you can use the `ChatMessage` class with structured `blocks`. This is the modern way to represent multi-modal content in LlamaIndex.

A `ChatMessage` can contain multiple `ContentBlock` objects, such as `TextBlock`, `ImageBlock`, `AudioBlock`, etc.

```python
from llama_index.core.llms import ChatMessage, TextBlock, ImageBlock

# Create a multi-modal message with text and an image
message = ChatMessage(
    role="user",
    blocks=[
...
# Access the cumulative text content (joined TextBlocks)
...
For backward compatibility, assigning a string to the `content` property will automatically wrap it in a `TextBlock` within the `blocks` list.
